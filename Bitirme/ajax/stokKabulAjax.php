<?php
	...
	function cikisKayitGuncelle($id,$query){
		$kontrol = new StokKabulKontrol();
		$kayit = $kontrol -> etkilenenKayitSayisi("SELECT id FROM hasta WHERE asiNo = ".$query['uID']." and tcNo = '".$query['tc']."' ");
		if ($kayit > 0){
			...
			echo "<script>alert('Yanlış Aşı Uygulaması. Lütfen Aşıyı Kontrol Edin.')</script>";
		}
	}
